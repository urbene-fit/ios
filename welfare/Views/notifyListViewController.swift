...
class notifyListViewController: UIViewController, UITableViewDataSource, UISearchBarDelegate, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let parameters = ["type": "pushList","login_token": LoginManager.sharedInstance.token]
            ...
            Alamofire.request("https://www.hyemo.com/push", method: .get, parameters: parameters,encoding: URLEncoding.default, headers: headers)
                .validate()
                .responseJSON { response in
                    ...
                    //                                }
                                                    ...
                                                    self.items.append(item.init(welf_name: parseResult.Message[i].welf_name ,title: parseResult.Message[i].title, content: parseResult.Message[i].content, update_date: self.date, welf_local : parseResult.Message[i].welf_local, pushId: parseResult.Message[i].pushId))
                                            }
...
//
//
//        Alamofire.request("https://www.hyemo.com/push", method: .get, parameters: ["type": "pushList","login_token": "123"])
//            .validate()
//            .responseJSON { response in
//
//                switch response.result {
...
//        self.navigationController?.pushViewController(RVC, animated: true)
        ...
        var push_id : String = String(items[indexPath.row].pushId)
        print("선택한 알림 번호 : \(push_id)")
        let parameters = ["type":"pushRecv", "pushId":push_id]
...
//    }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
            ...
            let deleteAction = UIContextualAction(style: .destructive, title:  "삭제", handler: { (ac:UIContextualAction, view:UIView, success:(Bool) -> Void) in
                ...
                //tableView.delete(IndexPath.self)
                // Reset state
                print("삭제")
                print(indexPath)
                
                //삭제 전에 알림리스트로부터 push id를 받아온다.
                var push_id : String = String(self.items[indexPath.row].pushId)

                
                //삭제 후 리로드
                self.items.remove(at: indexPath.row)
                //self.alramTbView.deleteRows(at: [indexPath], with: .none) 애니메이션 효과
                self.alramTbView.reloadData()
                ...
                print("선택한 알림 번호 : \(push_id)")
                let parameters = ["type":"delete", "pushId":push_id]
